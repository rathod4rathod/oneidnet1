...
//mar-11-2016 by venkatesh
    ...
    function statesuggestion() {
        ...
        $query = $db->custom("SELECT state_id,state_name FROM global_states_info WHERE state_name LIKE '%" . $searchTerm . "%'");
        /*foreach ($query as $queryval) {
            $data[] = $queryval['state_name'];
        }*/
        $states_arry=[];
        foreach ($query as $queryval) {
    ...
    function countrysuggestion() {
        ...
        $searchTerm = $_REQUEST['q'];
        $countries_arry=[];
        $query = $db->custom("SELECT country_name,country_id FROM iws_countries_info WHERE country_name LIKE '%" . $searchTerm . "%' ORDER BY country_name ASC");
        foreach ($query as $queryval) {
            $data['name'] = $queryval['country_name'];
            $data["id"] = $queryval['country_id'];
            array_push($countries_arry,$data);
        }
        echo json_encode($countries_arry);
    }
...
//written by Gouthami:Loading buzzin Vip promotion view 
    function buzzinVip() {
        $this->load->view('promotions/buzzin/vip_view');
    }
