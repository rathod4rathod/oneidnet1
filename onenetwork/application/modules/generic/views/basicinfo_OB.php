<div class="click_popUp" id="os_popup">

</div>
<?php
$this->load->module('templates');
$this->templates->header();
$this->templates->onenet_header();
?>

<div class="clr">&nbsp;</div>
<div class="new_wrapper">
    <div class="ondes_wrapper_main">
        <div class="ondes_module_container_wrap">
            <!--module_container start here-->
            <div class="ondes_wrapper_inner minheight600">
                <!--wrapper inner start here-->
                <div class="oneshop_getstarted mat30">
                    <div class="oneshop_getstarted_bgwrap mat40 mab30" style="position:relative; margin-top:-5%;">
                        <h1 class="normal fs24" style="width:880px; border-bottom:1px #ccc solid; padding:0 0 10px 0; margin:0 auto; line-height:32px;"> Official Badge </h1>
                        <?php
                            $i_url = $this->uri->total_segments();
                            $s_url = $this->uri->segment($i_url);
                            if ($s_url != "" && $i_url <= 3) {
                                                $sdata = explode("_", $s_url);
                                                $module = $sdata[0];
                                                $target = $sdata[1];

                            }
                        ?>
                        <div style="width:880px; padding:0 0 5px 0; margin:20px 0 0 0; line-height:24px;" class="bold borderbottom">
                            <span> <a href="<?php echo base_url().'official_badge'?>"> Official Badge </a> &nbsp;&nbsp;&gt;&nbsp;&nbsp; </span>
                            <span> <a href="#"> <?php echo $module;?> </a> &nbsp;&nbsp;&gt;&nbsp;&nbsp; </span>
                            <span> <a href="#"> <?php echo $target;?> </a> &nbsp;&nbsp;&nbsp;&nbsp;</span>
                        </div>
                        <p class="fs18 fll mat30 bgcolor3 wi100pstg"> <span class="fll"> <img  src="<?PHP echo base_url() . 'assets/images/next.png'; ?>" width="22" height="22"> </span> <span class="fll mal10"> Complete your Compaign </span>  </p>
                        <?php /*  if ($officialbadgedata[0]['rec_aid'] == 0) { */ ?>
                            <div class="mat20 fll" style="width:500px;">
                                <div class="click_tabs_wrap">
                                    <ul id="tabs">
                                        <li><a id="current" name="#tab1" class="oneiddev_budbase" href="<?PHP echo base_url() . 'budget_page' ?>" name="#tab1"> 1 </a> <p style="float:left; font-weight:bold; padding:0 0 0 0; margin:65px 0px 10px -85px;"> BASIC INFORMATION </p> </li>
                                        <li><a href="" name="#tab2" id="oneiddev_budthrd"> 2 </a> <p style="float:left; font-weight:bold; padding:0 0 0 0; margin:65px 0 10px -50px;"> PAYMENT</p>   </li>
                                    </ul>
                                </div>
                                <div id="content">
                                    <div style="display: block; margin-left:80px;" id="tab1">
                                        <p class="bold fs12 mat20">  BASIC INFORMATION  </p>

                                        <div class="basic_info">
                                            <?php
                                            $this->load->module('generic');
                                            $i_url = $this->uri->total_segments();
                                            $s_url = $this->uri->segment($i_url);
                                            $this->generic->basicFormViewOB($s_url);
                                            if ($s_url != "" && $i_url <= 3) {
                                                $sdata = explode("_", $s_url);
                                                $module = $sdata[0];
                                                $target = $sdata[1];
                                                if ($module == 'Netpro' && $target == 'Groups') {
                                                    $modulename = 'NETPRO';
                                                    $moduleentity = 'NETPRO_GROUPS';
                                                } else if ($module == 'Tunnel' && $target == 'Channel') {
                                                    $modulename = 'TUNNEL';
                                                    $moduleentity = 'TUNNEL_CHANNEL';
                                                } else if ($module == 'Oneshop' && $target == 'Store') {
                                                    $modulename = 'ONESHOP';
                                                    $moduleentity = 'ONESHOP_STORE';
                                                } else if ($module == 'Coffice' && $target == 'Company') {
                                                    $modulename = 'CORPORATE_OFFICE';
                                                    $moduleentity = 'COFFICE_COMPANYPROFILE';
                                                } else if ($module == 'Click' && $target == 'Page') {
                                                    $modulename = 'CLICK';
                                                    $moduleentity = 'CLICK_PAGE';
                                                } else if ($module == 'Click' && $target == 'Event') {
                                                    $modulename = 'CLICK';
                                                    $moduleentity = 'CLICK_EVENT';
                                                } else if ($module == 'Dealerx' && $target == 'Dealer') {
                                                    $modulename = 'DEALERX';
                                                    $moduleentity = 'DEALERX_DEALERPROFILE';
                                                }
                                            }
                                            $this->load->module("db_api");
                                            $sql="select rates from onetwork_official_badge_rates where module='$modulename' and entity_type='$moduleentity' and is_revised=0";
                                            $data = $this->db_api->custom($sql);
                                            ?>
                                            <input type="hidden" name="onenetwork_modulename" id="onenetwork_modulename" value="<?php echo $modulename; ?>">
                                            <input type="hidden" name="onenetwork_moduleentityname" id="onenetwork_moduleentityname" value="<?php echo $moduleentity; ?>">
                                            <input type="hidden" name="onenetwork_rates" id="onenetwork_rates" value="<?php echo $data[0]['rates'];?>">
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Brief History </p>
                                                <p>
<!--                                                    <input type="text" class="oneshop_productfield_textbox" name="onenetwork_description" id="onenetwork_description" onfocus="removeerror(this.id)"> -->
                                                    <textarea name="onenetwork_description" cols="50" rows="4" id="onenetwork_description" onfocus="removeerror(this.id)"></textarea>
                                                </p>
                                            </div>
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Website </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_website" id="onenetwork_website" onfocus="removeerror(this.id)" > </p>
                                            </div>
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Official EmailId for Correspondence</p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_emailid" id="onenetwork_emailid" onfocus="removeerror(this.id)" > </p>
                                            </div>
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Mobile Number / Landline No </p>
                                                <p><input type="text" id="isd_code" name="isd_code" style="width:60px;"/> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_mobileno" id="onenetwork_mobileno" onfocus="removeerror(this.id)" maxlength="15"> </p>
                                            </div>
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Address Line1</p>
                                                <p>
<!--                                                    <input type="text" class="oneshop_productfield_textbox" name="onenetwork_addressline1" id="onenetwork_addressline1" onfocus="removeerror(this.id)" maxlength="80"> -->
                                                    <textarea name="onenetwork_addressline1" id="onenetwork_addressline1" cols="50" rows="4" onfocus="removeerror(this.id)"></textarea>
                                                </p>
                                            </div>
                                            <div class=" mab10 mat10 form_width">
                                                <p class="label_name mat20 fll"> Address Line2 </p>
                                                <p>
<!--                                                    <input type="text" class="oneshop_productfield_textbox" name="onenetwork_addressline2" id="onenetwork_addressline2" onfocus="removeerror(this.id)" maxlength="80">-->
                                                    <textarea name="onenetwork_addressline2" id="onenetwork_addressline2" cols="50" rows="4" onfocus="removeerror(this.id)"></textarea>
                                                </p>
                                            </div>
                                            <div class="mab10 mat10 form_width">
                                                <p class="label_name"> Country </p>
                                                <p>
                                                    <select name="onenetwork_countries" id="onenetwork_countries"  class="flight_searchfield_adult_and_childcontainer" onchange="removeerror(this.id)">
                                                        <option value="">select country</option>
                                                            <?php foreach ($countriesList as $countrydata) { ?>
                                                                    <option value="<?php echo $countrydata[country_id]; ?>"><?php echo $countrydata['country_name']; ?></option>
                                                            <?php } ?>
                                                    </select>
                                                </p>
                                            </div>
                                            <div class="mab10 mat10 form_width">
                                                <p class="label_name"> State </p>
                                                <p>
                                                    <select name="onenetwork_states" id="onenetwork_states" class="flight_searchfield_adult_and_childcontainer" onchange="removeerror(this.id)">
                                                        <option value="">select state</option>
                                                    </select>
                                                </p>
                                            </div>
                                            <div class="mab10 mat10 form_width">
                                                <p class="label_name"> City</p>
                                                <p>
                                                    <select name="onenetwork_cities" id="onenetwork_cities" class="flight_searchfield_adult_and_childcontainer" onchange="removeerror(this.id)">
                                                        <option value="">select city</option>
                                                    </select>
                                                </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Zipcode </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_zipcode" id="onenetwork_zipcode" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Twitter Url </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_twitter" id="onenetwork_twitter" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Facebook Url </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_facebook" id="onenetwork_facebook" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Instagram Url </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_instagram" id="onenetwork_instagram" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Linkedin Url </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_linkedin" id="onenetwork_linkedin" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                            <div class="fll  form_width">
                                                <p class="label_name"> Google Plus Url </p>
                                                <p> <input type="text" class="oneshop_productfield_textbox" name="onenetwork_googleplus" id="onenetwork_googleplus" onfocus="removeerror(this.id)"> </p>
                                            </div>
                                        </div>
                                        <div class="clearfix"></div>
                                        <div class="mat20 mar10"> <input type="button" value="NEXT" class="np_des_checkout_btn" name="button" id="onenetwork_generic_official_badgesubmit">  </div>
                                    </div>

                                   <div  id="tab2">
    <p class="bold fs12 mat20" style="margin-left:50px;">   CONFIRM CAMPAIGN DETAILS </p>
    <div class="details">
        <div class="details_left_nox">

            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Description</strong> </p>
                <span id="desc"></span>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Website </strong> </p>
                <p id="website"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Official Emailid </strong> </p>
                <p id="mailid"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Official Mobile Number</strong> </p>
                <p id="mobileno"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Address Line1</strong> </p>
                <p id="addline1"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Address Line2 </strong> </p>
                <p id="addline2"> </p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Country </strong> </p>
                <p id="country"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong> State </strong> </p>
                <p id="state"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>City </strong> </p>
                <p id="city"></p>
            </div>
            <div class="fll mab10 mat10 form_width_cards_new">
                <p class="label_name"> <strong>Zipcode </strong> </p>
                <p id="zipcode"></p>
            </div>

        </div>



        <div class="onenetwork_homepage_payeenewbox mat30">

            <div class="amount_msg"> The domain lpsum.com may be for sale. Click here to make an offer may be for sale. Click here to make an offer or call 877-588-1085 to speak with one of our domain experts. You have to <span class="red bold red_bold1"> </span> towards ONEIDNET account. </div>
            <div class="amount_msg_clarity">Are you ready to pay? <span class="red bold red_blod"> </span></div>
            <div class="clearfix"></div>
            <div class="amount_msg_buttons"> <button name=" " id="payment_yes" class="button_new_payeebtn" value="YES" type="button">YES</button>
                <button name=" " class="button_new_payeebtn" value="NO" type="button">NO</button>
            </div>
        </div>
    </div>
<!--    <p class="bold fs12 mat30 fll" style="margin-left:0px;">   CONFIRM CAMPAIGN DETAILS </p>-->
<!--    <div class="onenetwork_homepage_right_box_payment">
        <div class="fll mab10 mat10 form_width">
            <p class="label_name"> Choose Card </p>
            <p> <select name="privacy" class="flight_searchfield_adult_and_childcontainer">
                    <option value="">New Card</option>

                </select> </p>
        </div>
        <div class="cards_here">
            <div class="card_left">
                <div class="fll mab10 mat10 form_width_cards">
                    <p class="label_name"> <strong>Card Number</strong> </p>
                    <p> XXXX XXXX XXXX XXXX</p>
                </div>
                <div class="fll mab10 mat10 form_width_cards">
                    <p class="label_name"> <strong>Usage Type</strong> </p>
                    <p> INR</p>
                </div>
            </div>
            <div class="card_right">

                <div class="fll mab10 mat10 form_width_cards">
                    <p class="label_name"> <strong>Card Number</strong> </p>
                    <p> XXXX XXXX XXXX XXXX</p>
                </div>


                <div class="fll mab10 mat10 form_width_cards">
                    <p class="label_name"> <strong>Usage Type</strong> </p>
                    <p> INR</p>
                </div>
            </div>

            <div class="clearfix"></div>
            <div class="button_right clearfix fll">
                <button name=" " class="button_new2 clearfix flr" value="Confirm" type="button">Confirm</button>
            </div>
        </div>
    </div>-->
</div>
                                </div>
                            </div>
                            <div id="onedev_content" class="hotel_pachagesummary_box mab10 mat40 fll">
                                <div id="tab1right">
                                    <p class="bgcolor3 bold fs14"> Quick Links ! </p>
                                    <ul>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Hasn't Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Shipment hasn't Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Hasn't Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Shipment hasn't Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Hasn't Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> This Shipment Initiated Yet. </li>
                                        <li><p class="fll mat3 mar5"> <img width="10" height="14" src="<?php echo base_url() . "assets/images/downarrow2.png"; ?>"> </p> Shipment hasn't Initiated Yet. </li>
                                    </ul>
                                </div>



                            </div>

//<?php
//} else {
//    echo "Your already register";
//}
//?>
                    </div>
                </div>
            </div>
            <!--wrapper inner closed here-->
        </div>
        <!--module container end here-->
    </div>
    <!--wrapper main closed here-->
                        <?php
                        $this->templates->right_container();
                        ?>
</div>


<div class="clr"></div>

<!--footer start here-->
    <?php
    $this->templates->footer();
    ?>

<!--footer closed here-->
<script type="text/javascript" src="<?php echo base_url(); ?>assets/js/tab.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>assets/microjs/tunnelchannel.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>assets/microjs/officialbadge.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>assets/microjs/promotionPayment.js"></script>

