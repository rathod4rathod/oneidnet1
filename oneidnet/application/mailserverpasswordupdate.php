...
class oneidnet_rc { 
	...
	function db_connect( $params ) {		
		echo $connection = mysqli_connect( $params['HOST'] , $params['USER'], $params['PASSWORD'], $params['DATABASE'] ) or $this->errorLogComposer( mysqli_connect_errno($connection),"DB CONNECTION ERROR" );
		if($connection){
			return $connection;
		} else {
		...
		if( !$this->connection ) {
			...
			$affected_result = mysqli_affected_rows( $this->connection );
				...
				if( $affected_result ) {
					...
					if( strtolower($crud_type) == 'select'  ) {
	...
	function addMailbox( $username, $full_name, $password_hash, $quota_in_mb ) {
		...
		$storage_node = $domain."/".substr($username, 0, 1)."/".substr($username, 1, 1)."/".substr($username, 2, 1)."/".$username."-"
		  ...
		  $mailbox_query = "INSERT INTO mailbox (username, password, name, storagebasedirectory,storagenode, maildir, quota, domain, active, local_part, created)".
             "VALUES ('{$email}', '{$password_hash}', '{$full_name}',
                     '/var/vmail','vmail1', '{$storage_node}', '{$quota_in_mb}', '{$domain}', '1','{$username}', NOW())";
        
        $alias_query = "INSERT INTO alias (address, goto, domain, created, active) VALUES ('{$email}', '{$email}','{$domain}', NOW(), 1)";
		...
		if( $mailbox_response_flag1 && $mailbox_response_flag2 ) {			
			// Means Both the query executed successfully
...
<?php
$dbhost = 'mightymysqli.oneidnet.com';
   $dbuser = 'oneidnetSuper';
   $dbpass = 'OneidnetAppsNations786@Zeferrius';
   ...
   $sql = 'SELECT profile_id,username,concat(`first_name`," ",`middle_name`," ",`last_name`) as fullname,password_hash FROM iws_profiles where is_verified="1" order by profile_id';
   ...
   while($row = mysql_fetch_array($retval, MYSQL_ASSOC)) {
	   ...
	   echo $username=$row["password_hash"]."</br>";
	   $bd=$oneidnetRCobj->changeMailboxPassword( $username=$row["username"], $password_hash=$row["password_hash"]);
		//$bd = $oneidnetRCobj->addMailbox( $username=$row["username"], $full_name=$row["fullname"], $password_hash=$row["password_hash"], $quota_in_mb=1024 ) ;	  
   }
