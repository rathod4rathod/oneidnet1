        ...
        if ($rlt != 0) {
          $regobj= $this->load->module("registration");  
            if($rlt[0]["password_hash"]=="{SSHA512}".base64_encode(hash('sha512', $currentpw.$regobj->salt(), true).$regobj->salt())){
                ...
                $mail360_hash_pass=$regobj->encrypt($_REQUEST["new_pw"]);
                $fields = ["password_hash" =>"{SSHA512}".base64_encode(hash('sha512', $newpw.$regobj->salt(), true).$regobj->salt()),"360mail_key"=>$mail360_hash_pass];
                ...
                if($uprlt==1){
                     ...
                     $this->load->module("login");
                     $_SESSION['key']=$mail360_hash_pass;
    ...
    function cropimg() {
        ...
        if ($_REQUEST['x']) {
     	 ...
     	 $ini_filename ='temp/' . $_REQUEST["temp_name"];
            ...
            if ($rlt == 1) {
				$blog_result=$this->db_api->select("profile_pic_str","blog_users_details","user_id_fk='".$this->user_id()."'");
				$blog_ppic_arry=json_decode($blog_result[0]["profile_pic_str"],TRUE);
				//print_r($blog_result);
				foreach($blog_ppic_arry as $bkey=>$bval){
				  if($bkey=="ONEIDNET"){
					$blog_ppic_arry[$bkey]=$_REQUEST['temp_name'];
				  }
				}
				$blog_ppic_json=json_encode($blog_ppic_arry);
				$bdata_arry=array("profile_pic_str"=>$blog_ppic_json);
				$this->db_api->update($bdata_arry,"blog_users_details","user_id_fk='".$this->user_id()."'");
                $this->memcach_reset();
            }
...
function contacts(){
    ...
    if(isset($_REQUEST["sval"])){
        $data["sval"]=$_REQUEST["sval"];
    }

$this->load->view("basicinfo/searchContact",$data);
}
