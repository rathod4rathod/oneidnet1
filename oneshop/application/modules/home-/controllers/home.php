    ...
    function user_Profile_Page() {
       ...
       $sql=" SELECT profile_id_fk ,os.profile_name  ,os.profile_img ,os.profile_cover_img ,iw.user_id  FROM os_user_details as os
              ...
              inner join  iws_profiles as iw  on os.profile_id_fk = iw.profile_id
               where os.profile_id_fk=".$this->get_UserId() ;
...
//25-06-2015 by venkatesh
    ...
    function privacy_store($store_code) {
        $this->load->module("db_api");
        $store_query = "SELECT * FROM oshop_stores os  WHERE os.store_code = '" . $store_code . "'";
        $store_res = $this->db_api->custom($store_query);
        $store_id = $store_res[0]["store_aid"];
        $data['store_details'] = $store_res;
    ...
    function store_orders($store_code) {
        ...
        $data['store_details'] = $stores_result;
        $this->load->module("db_api");
        $store_query = "SELECT order_aid,order_code,time,status FROM oshop_orders WHERE store_id_fk =(select store_aid from oshop_stores where store_code='$store_code') order by time desc";
        $store_res = $this->db_api->custom($store_query);
    ...
    function searchOrder(){
         ...
         if($searchtxt=='onemonth'){
           $query ="SELECT order_aid,order_code,time,status FROM `oshop_orders` where  DATE(time)>=DATE_SUB(CURDATE(), INTERVAL  1 MONTH) and store_id_fk =(select store_aid from oshop_stores where store_code='$storecode') order by time descs";
         }elseif($searchtxt=='oneweek'){
             $query ="SELECT order_aid,order_code,time,status  FROM `oshop_orders` where DATE(time)>=DATE_SUB(CURDATE(), INTERVAL 7 DAY) and store_id_fk =(select store_aid from oshop_stores where store_code='$storecode') order by time desc";

         }elseif($searchtxt=='oneday'){
             $query ="SELECT order_aid,order_code,time,status FROM `oshop_orders` where DATE(time)=DATE_SUB(CURDATE(), INTERVAL 1 DAY) and store_id_fk =(select store_aid from oshop_stores where store_code='$storecode') order by time desc ";

         }else if($searchtxt=='all'){
              $query = "SELECT order_aid,order_code,time,status FROM oshop_orders WHERE store_id_fk =(select store_aid from oshop_stores where store_code='$storecode') order by time desc";
    ...
    function product_Detail_View($store_code,$product_code) {
        ...
        $loggedin_res = $this->db_api->select("img_path", "iws_profiles", "profile_id=" . $user_id);
        $data["loggedinuser_img"] = $loggedin_res[0]["img_path"];
        $products_res = $this->db_api->custom("select oas.created_by,op.description,op.product_category_id_fk,oas.store_aid as store_id,oas.store_code as store_code,oas.store_name as store_name,oas.profile_image_path as logo_path, op.product_aid,op.total_views, op.product_code,op.cost_price, op.sale_price, op.product_name, op.discount, op.specification,osc.groups as category_name, osc.category_name as subcategory_name,osc.product as category,op.primary_image as prod_img1,op.secondary_image as prod_img2,op.tertiary_image as prod_img3,op.quaternary_image as prod_img4 from oshop_products op left join oshop_stores oas on op.store_id_fk=oas.store_aid left join oshop_categories osc on op.product_category_id_fk=osc.category_id_fk where op.product_aid=" . $product_id);
    ...
    function getUsrActivityLogTpl(){
      ...
      $user_data=array();
      $data["user_details"]=$dbapi->custom("SELECT profile_name,profile_img,user_id FROM os_user_details users INNER JOIN iws_profiles profiles ON users.profile_id_fk=profiles.profile_id WHERE profile_id_fk=".$this->get_UserId());
      $streviews_result=$dbapi->custom("SELECT store_id_fk,store_name,store_code,follower_count,commented_on,profile_image_path FROM oshop_store_reviews sreviews INNER JOIN oshop_stores stores ON sreviews.store_id_fk=stores.store_aid ORDER BY commented_on DESC WHERE user_id_fk=".$this->get_UserId()." LIMIT $start,1");
      $stores_result=$dbapi->custom("SELECT store_code,store_name,profile_image_path,registered_on,follower_count FROM oshop_stores WHERE created_by=".$this->get_UserId()." ORDER BY regsitered_on LIMIT $start,1");
      $prreviews_result=$dbapi->custom("SELECT product_aid,product_name,product_code,store_code,profile_image_path,primary_image,rating_on,rating FROM oshop_product_reviews previews INNER JOIN oshop_products prods ON previews.product_aid_fk=prods.product_aid INNER JOIN oshop_stores stores ON prods.store_id_fk=stores.store_aid WHERE user_id_fk=".$this->get_UserId()." ORDER BY rating_on DESC LIMIT $start,1");
      $wishlist_result=$dbapi->custom("SELECT sprods.added_on,product_name,product_code,product_aid,store_code,primary_image FROM `oshop_saved_products` sprods INNER JOIN oshop_products prods ON sprods.product_id_fk=prods.product_aid INNER JOIN oshop_stores stores ON prods.store_id_fk=stores.store_aid WHERE profile_id=".$this->get_UserId()." ORDER BY sprods.added_on DESC LIMIT $start,1");
      $sfollow_result=$dbapi->custom("SELECT store_name,store_code,store_aid,following_since,profile_image_path,follower_count FROM `oshop_followings` follow INNER JOIN oshop_stores stores ON follow.store_id_fk=stores.store_aid WHERE user_id_fk=".$this->get_UserId()." ORDER BY following_since DESC LIMIT $start,1");
      $ufollow_result=$dbapi->custom("SELECT `profile_img`,`profile_name`,user_id,date FROM `os_user_details` users INNER JOIN oshop_explore explore ON users. `profile_id_fk`=explore.user_id_fk INNER JOIN iws_profiles profiles ON explore.user_id_fk=profiles.profile_id WHERE associated_id_fk=".$this->get_UserId()." ORDER BY date DESC LIMIT $start,1");
      $user_data["store_reviews"]=$streviews_result;
      $user_data["product_reviews"]=$prreviews_result;
      $user_data["saved_products"]=$wishlist_result;
      $user_data["stores"]=$stores_result;
      $user_data["store_follows"]=$sfollow_result;
      $user_data["user_follows"]=$ufollow_result;
      //print_r($user_data);
      $data["activity_data"]=$user_data;
    ...
    function frndsActivityLogTpl(){
      ...
      if($frnds_str!==""){
        $user_data=array();
        //echo "SELECT `profile_img` AS profile_pic,`user_id`,`profile_name`,user_id,date FROM `os_user_details` users INNER JOIN oshop_explore explore ON users.`profile_id_fk`=explore.user_id_fk INNER JOIN iws_profiles profiles ON explore.user_id_fk=profiles.profile_id WHERE associated_id_fk IN ($user_id) ORDER BY date DESC LIMIT $start,2";
        //$data["user_details"]=$dbapi->custom("SELECT profile_name,profile_img,user_id FROM os_user_details users INNER JOIN iws_profiles profiles ON users.profile_id_fk=profiles.profile_id WHERE profile_id_fk IN ($frnds_str)");
        $streviews_result=$dbapi->custom("SELECT store_id_fk,store_name,store_code,follower_count,commented_on,profile_image_path,(SELECT profile_name FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_name,(SELECT profile_img FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_pic,(SELECT user_id FROM iws_profiles WHERE profile_id=user_id_fk) AS user_id FROM oshop_store_reviews sreviews INNER JOIN oshop_stores stores ON sreviews.store_id_fk=stores.store_aid ORDER BY commented_on DESC WHERE user_id_fk IN ($frnds_str) LIMIT $start,2");
        $stores_result=$dbapi->custom("SELECT store_code,store_name,profile_image_path,registered_on,follower_count,(SELECT profile_name FROM os_user_details WHERE profile_id_fk=created_by) AS profile_name,(SELECT profile_img FROM os_user_details WHERE profile_id_fk=created_by) AS profile_pic,(SELECT user_id FROM iws_profiles WHERE profile_id=created_by) AS user_id FROM oshop_stores WHERE created_by IN ($frnds_str) ORDER BY regsitered_on LIMIT $start,2");
        $prreviews_result=$dbapi->custom("SELECT product_aid,product_name,product_code,store_code,profile_image_path,primary_image,rating_on,rating,(SELECT profile_name FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_name,(SELECT profile_img FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_pic,(SELECT user_id FROM iws_profiles WHERE profile_id=user_id_fk) AS user_id FROM oshop_product_reviews previews INNER JOIN oshop_products prods ON previews.product_aid_fk=prods.product_aid INNER JOIN oshop_stores stores ON prods.store_id_fk=stores.store_aid WHERE user_id_fk IN ($frnds_str) ORDER BY rating_on DESC LIMIT $start,2");
        $wishlist_result=$dbapi->custom("SELECT sprods.added_on,product_name,product_code,product_aid,store_code,primary_image,(SELECT profile_name FROM os_user_details WHERE profile_id_fk=profile_id) AS profile_name,(SELECT profile_img FROM os_user_details WHERE profile_id_fk=profile_id) AS profile_pic,(SELECT user_id FROM iws_profiles WHERE profile_id=sprods.profile_id) AS user_id FROM `oshop_saved_products` sprods INNER JOIN oshop_products prods ON sprods.product_id_fk=prods.product_aid INNER JOIN oshop_stores stores ON prods.store_id_fk=stores.store_aid WHERE profile_id IN ($frnds_str) ORDER BY sprods.added_on DESC LIMIT $start,2");
        $sfollow_result=$dbapi->custom("SELECT store_name,store_code,store_aid,following_since,profile_image_path,follower_count,(SELECT profile_name FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_name,(SELECT profile_img FROM os_user_details WHERE profile_id_fk=user_id_fk) AS profile_pic,(SELECT user_id FROM iws_profiles WHERE profile_id=user_id_fk) AS user_id FROM `oshop_followings` follow INNER JOIN oshop_stores stores ON follow.store_id_fk=stores.store_aid WHERE user_id_fk IN ($frnds_str) ORDER BY following_since DESC LIMIT $start,2");
        $ufollow_result=$dbapi->custom("SELECT `profile_img` AS profile_pic,`user_id`,`profile_name`,user_id,date FROM `os_user_details` users INNER JOIN oshop_explore explore ON users.`profile_id_fk`=explore.user_id_fk INNER JOIN iws_profiles profiles ON explore.user_id_fk=profiles.profile_id WHERE associated_id_fk IN ($frnds_str) ORDER BY date DESC LIMIT $start,2");
        $user_data["store_reviews"]=$streviews_result;
        $user_data["product_reviews"]=$prreviews_result;
        $user_data["saved_products"]=$wishlist_result;
        $user_data["stores"]=$stores_result;
