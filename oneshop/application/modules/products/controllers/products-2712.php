...
class products extends CI_Controller {
    ...
    function products_search_result($catid = NULL, $subcatid = NULL, $itemid = NULL, $searchparam = NULL,$store_id='') {
        ...
        if ($searchparam) {
            $s_where = $s_where . " prods.product_name like '%" . $searchparam . "%' and ";
        }
        ...
        $store_details=$dbobj->select("created_by","oshop_stores","store_aid=".$store_id);
        $store_owner="no";
    ...
    function store_products_result($storeid = NULL) {
        ...
        $s_where = $s_where . "1";
        $s_where = $s_where . " LIMIT $start,$records_per_page";
        $s_query = "SELECT prods.product_name,prods.product_aid,prods.cost_price as price,prods.primary_image,prods.secondary_image,prods.tertiary_image,prods.quaternary_image FROM oshop_filtration_electronics osf right join oshop_products prods on osf.product_aid_fk = prods.product_aid WHERE " . $s_where;
        $os_products_res = $dbobj->custom($s_query);
    ...
    function product_insertion() {
      ...
      if($result[0]["user_id_fk"]!==$userid && $result[0]["role"]=="ORDER_MANAGER"){
        ...
        if($this->checkProductExists($_REQUEST["Product_Name"])){
          ...
          //$this->createProdDirectory($result[0]["store_code"]);
          
          $img_paths="";
          $category = $_REQUEST["Sub_Category"];
          $selectcatid = $this->db_api->select("category_id_fk", "oshop_categories", "product like '" . $category . "'");
          $hex_code = Uuid::v4();
          $a_data = array("cost_price" => $_REQUEST["Cost_Price"],"sale_price" => $_REQUEST["Sell_Price"],"product_name" =>$_REQUEST['Product_Name'],"hex_code" => $hex_code,"discount" => $_REQUEST['Discount'],"store_id_fk" => $i_store_id,"product_category_id_fk" => $selectcatid[0]['category_id_fk'],"description" => $_REQUEST['Description'],"added_by"=>$this->get_UserId());
