...
while(my($str,$str1)=each @data){
	...
	if($ret eq "greater"){
		$sql="UPDATE os_all_store SET is_package_active='No' WHERE store_aid=$storeid";
		#print $sql;
		$affectedrows=$dbh->do($sql);
		if($affectedrows){
			$sql="UPDATE os_store_renewals_info SET is_renewed=0 WHERE store_id_fk='$storeid'";
			$affectedrows=$dbh->do($sql);
			$cmd="sed -i '/$storeid/d' $filename";
			system($cmd);
		}
